(ns deckcrew.actor
  "DeckCrewActor — the ISCO-08 8350 Ships' Deck Crews and Related
  Workers crew-scheduling/logistics actor as a
  `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one crew-scheduling/logistics operation
  ...
  The unconditional invariant: the DeckCrewAdvisor can never directly
  commit a record the DeckCrewGovernor refuses — every
  commit-record! call is gated behind `:decide`. This actor
  coordinates crew scheduling and logistics ONLY: it never performs a
  deck operation itself, and no path through this graph can finalize
  a mooring/cargo-handling operational decision or a heavy-weather
  deck-work go/no-go decision — the closed op-allowlist and the
  governor's scope exclusion make that structurally impossible, not
  merely a policy the advisor is asked to follow."
...
(defn build-graph
  "Build a compiled DeckCrewActor graph. `store` implements
  `deckcrew.store/Store`. `advisor` implements
  `deckcrew.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn approve!
  ...
  LOGISTICS proposal, not a deck operation, which this actor never
  performs)."
  [graph thread-id]
